...
void SQLDeleteWhere(){
    ...
    for(int i = 0; i < search_num; i++){
        ...
        printf("Resposta para a deleção %d\n", i+1);
        found = SearchDeleteBinaryFile(read_file_name, index_file_name, index_parameter);
        if(found == 0){ //
            printf("Registro inexistente.\n");
        }
        else{
            printf("refazendo index\n");
            // colocar create index aqui
        }
        int64_t offset = 0;
